...
def _validate_external_uri(external_uri: str,
  ...
  """Checks if the external uri is not the subdirectory of the pipeline root."""
  if external_uri == RESOLVED_AT_RUNTIME:
    return

  if pipeline_root and pipeline_root == os.path.commonpath(
      [pipeline_root, os.path.realpath(external_uri)]):
    raise ValueError('The uri of the external artifact must be outside of the'
                     'base directory of the pipeline.')
